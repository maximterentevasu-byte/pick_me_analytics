...
function weekRange(){
  ...
  return {
    ...
    endStr:new Date(end-86400000).toISOString().slice(0,10)
  };
}
...
async function getStats(client,channel,range){
  ...
  const posts=raw.filter(m=>{
    const d=toDate(m);
    if(!d) return false;
    return d>=range.start && d<range.end && !m.action;
  });
  ...
  const quality=Number((erViews*0.5 + erAct*0.3 + virality*0.2).toFixed(2));
...
async function main(){
  console.log("хочу PRO аналитику");
